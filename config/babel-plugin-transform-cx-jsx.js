...
function objectKeyIdentifier(t, name) {
   if (name.indexOf('-') >= 0) return t.stringLiteral(name);
...
function property(t, name, value, options) {
   if (name.namespace && name.namespace.name) {
      return t.objectProperty(
         objectKeyIdentifier(t, name.namespace.name),
         bindExprTplObject(t, name.name.name, value, options)
      );
   }
...
function elementName(name) {
   ...
   if (name.object) return elementName(name.object) + '.' + elementName(name.property);
   throw new Error('Could not calculate name.');
}

function processAttribute(t, attribute, options) {
   if (attribute.type === 'JSXAttribute') {
      if (attribute.value == null) return property(t, attribute.name, t.booleanLiteral(true), options);

      return property(
         t,
         attribute.name,
         processChild(t, attribute.value, { root: false, scope: options.scope }),
         options
      );
   }
...
function processChild(t, child, options, preserveWhitespace) {
   ...
   switch (child.type) {
      ...
      case 'JSXText':
         if (preserveWhitespace) return t.stringLiteral(child.value);
...
function processElement(t, element, options) {
   if (element.type === 'JSXElement') {
      if (element.openingElement) {
         ...
         if (tagName != 'cx' && tagName != 'Cx' && options.root) return false;
            ...
            if (tagName == 'Cx') {
               if (children && children.length > 0)
                  element.openingElement.attributes.push(
                     t.jSXAttribute(t.jSXIdentifier('items'), t.jSXExpressionContainer(t.arrayExpression(children)))
                  );
               element.children = [];
               return element;
            }

            if (children && children.length > 1) return t.arrayExpression(children);
         ...
         if (dotIndex != -1)
            attrs.push(
               t.objectProperty(
                  t.identifier('$type'),
                  t.memberExpression(
                     t.identifier(tagName.substr(0, dotIndex)),
                     t.identifier(tagName.substring(dotIndex + 1))
                  )
               )
            );
         ...
         } else attrs.push(t.objectProperty(t.identifier('$type'), t.identifier(tagName)));
         ...
         let spread = [],
            preserveWhitespace =
               ...
               (options.scope.opts.trimWhitespaceExceptions &&
                  options.scope.opts.trimWhitespaceExceptions.indexOf(element.openingElement.name.name) != -1);
         ...
         if (spread.length > 0) attrs.push(t.objectProperty(t.identifier('jsxSpread'), t.arrayExpression(spread)));

         if (attrNames.length > 0) {
            attrs.push(
               t.objectProperty(
                  t.identifier('jsxAttributes'),
                  t.arrayExpression(
         ...
         if (element.children != null && element.children.length) {
            ...
            if (children.length) attrs.push(t.objectProperty(t.identifier('children'), t.arrayExpression(children)));
         }
      ...
      visitor: {
            ...
            enter: function (path, scope) {
               scope.$cx = {
                  ...
                  addImport: function (name, importPath) {
                     if (!scope.$cx.imports[name]) scope.$cx.imports[name] = { status: 'missing', path: importPath };
                  },
               };
            },
            exit: function (path, scope) {
               if (scope.$cx) {
                  Object.keys(scope.$cx.imports).forEach((name) => {
                     if (scope.$cx.imports[name].status != 'missing') return;

                     let identifier = t.identifier(name);
                     let importDeclaration = t.importDeclaration(
                        [t.importSpecifier(identifier, identifier)],
                        t.stringLiteral(scope.$cx.imports[name].path)
                     );
         ...
         ArrowFunctionExpression: function (path, scope) {
            ...
            if (
               ...
               node.body.openingElement.name.name === 'cx' &&
               !node.markedAsFunctionalComponentType
            ) {
